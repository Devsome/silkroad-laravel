<form method="post" action="#" id="generalReleaseForm">
    ...
    <div class="modal-body">
        <div class="form-group">
            <label for="inp-name">
                {{ __('backend/tickets.update-modal.status') }}
            </label>
            <input type="text"
              name="name"
              id="inp-name"
        ...
        <div class="form-group">
            ...
            <select class="form-control @error('color') is-invalid @enderror" name="color" id="inp-color" name="type">
                @forelse($colors as $key => $color)
                    <option value="{{ $key }}" @if($status)@if($status->color === $key) selected @endif @endif>
                        {{ $color }}
                    </option>
                @empty
...
<script>
    $(document).ready(function () {
        $('#generalReleaseForm').submit(function (e) {
            ...
            $.post( '@if($status){{ route('ticket-status-update', ['id' => $status->id]) }}@else{{ route('ticket-status-create') }}@endif',
