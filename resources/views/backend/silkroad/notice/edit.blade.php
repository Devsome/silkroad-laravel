...
@section('theme::backend-content')
    ...
    <div class="container-fluid">
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">{{ __('backend/notice.title-edit') }}</h1>
        </div>
        <div class="row">
            <div class="col-12">
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">{{ __('backend/notice.edit') }}</h6>
                    </div>
                    <div class="card-body">
                        @if (count($errors) > 0)
                        ...
                        @endif
                        <form method="POST" action="{{ route('sro-notice-patch-backend', ['id' => $notice->ID]) }}">
                            @csrf
                            <div class="form-group row">
                                <div class="col-6">
                                    ...
                                    <input type="text"
                                           ...
                                           name="title" id="title"
                                           aria-describedby="titleHelp" value="{{ $notice->Subject }}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control{{ $errors->has('body') ? ' is-invalid' : '' }}"
                                              name="body" id="body" rows="6">{{ $notice->Article }}</textarea>
                                </div>
                            </div>
