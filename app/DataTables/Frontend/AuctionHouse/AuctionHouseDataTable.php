...
class AuctionHouseDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->editColumn('item_data', static function ($data = '$query') {
                return '
                    <div id="selectInventory">
                        <div class="itemslot float-none mx-auto">
                            <div class="image"
                            style="background:url(' . $data->getItemInformation->imgpath . ')no-repeat;
                            background-size: 34px 34px !important; width: 34px; height: 34px !important;"
                            data-iteminfo="1">
                                <span class="qinfo">
                        ...
                        <div class="itemInfo">
                        ' . (($data->getItemInformation->data) ?
                        htmlspecialchars_decode(stripslashes($data->getItemInformation->data)) : '') . '
                        </div>
                    </div>
                ';
            })
            ->editColumn('name', static function ($data = '$query') {
                return '
            ...
            ->editColumn('price', static function ($data = '$query') {
                return number_format($data->price, 0, ',', '.');
            })
            ->editColumn('price_instead', static function ($data = '$query') {
                return number_format($data->price_instead, 0, ',', '.');
            })
            ->editColumn('until', static function ($data = '$query') {
            ...
            ->addColumn('actions', function ($data = '$query') {
                if ($this->mode_type && $this->mode_type === 'own') {
                    return
                        '<button class="btn btn-danger btn-circle btn-sm" onclick="DeleteData(' . $data->id . ')">' .
