...
class TicketsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->editColumn('updated_at', static function ($data = '$query') {
                return Carbon::make($data->updated_at)->diffForHumans();
            })
            ->addColumn('actions', static function ($data = '$query') {
                return '<a href="' . route('home-tickets-show', ['id' => $data->id]) .
                    '" class="btn btn-primary btn-circle btn-sm"><i class="fa fa-eye"></i></a>';
            })
            ->escapeColumns()
            ->rawColumns([
                'updated_at',
