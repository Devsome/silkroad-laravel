...
class AuctionsHouseController extends Controller
{
    ...
    public function index(AuctionHouseDataTable $dataTable, $mode = 'all')
    {
        ...
        return $dataTable->with(['mode' => $mode])
            ->render('theme::frontend.auctionshouse.index', ['mode' => $mode, 'filter' => 'none']);
    }
    ...
    {
        return $dataTable->with(['mode' => 'filter', 'type' => $type])
            ->render('theme::frontend.auctionshouse.index', ['mode' => 'filter', 'filter' => $type]);
    }
